...
/**
 * 接收输入参数映射,查找有效的FASTA输入,并返回一个通道,
 * 其元素为 `[metamap, assembly.fasta | null]`。
 * 对于没有匹配条形码目录的样本表条目,第二项为 `null`。
 * 这里不会使用 `fastcat`。如果存在多个FASTA文件,不会合并它们。
 *
 * @param arguments: 包含以下内容的参数映射
 *  - "input": 路径可以是: (i) 输入FASTA文件, (ii) 包含FASTA文件的顶级目录,
 *    (iii) 包含子目录的目录,子目录中包含FASTA文件
 *  - "sample": 用于命名单个样本的字符串
 *  - "sample_sheet": CSV样本表的路径
 *  - "analyse_unclassified": 是否保留未分类的读数的布尔值
 *  - "required_sample_types": 样本表中所需的样本类型列表
 *  - "watch_path": 是否使用 `watchPath` 并以流模式运行的布尔值
 * @return: `[Map(alias, barcode, type, ...), Path|null]` 形式的通道。
 *  第一个元素是包含元数据的映射,第二个是 `.fasta` 文件的路径。
 *  对于没有相应条形码目录的样本表条目,第二个元素为 `null`。
 */
def fasta_ingress(Map arguments)
{
    // 检查参数
    Map margs = parse_arguments("fasta_ingress", arguments)

    ArrayList fasta_extensions = [".fasta", ".fa", ".fna"]

    // `watch_path` 将在 `get_valid_inputs()` 中处理
    def input = get_valid_inputs(margs, fasta_extensions)

    def ch_result = input.files.mix(input.dirs)
    | map { meta, path -> 
        if (file(path).isDirectory()) {
            def fasta_files = get_target_files_in_dir(path, fasta_extensions)
            fasta_files.collect { fasta -> [meta, fasta] }
        } else {
            [[meta, path]]
        }
    }
